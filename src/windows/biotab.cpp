...
Biotab::Biotab(QWidget *parent) : QMainWindow(parent) {
...
void Biotab::setupUI() {
    ...
    // background widgets
    bioboard_widget = new QWidget(central);
    bioboard_widget->setGeometry(0, 0, central->width(), central->height());

    // widgets
    bioboard = new Bioboard(bioboard_widget);
}

void Biotab::setupTabChange() {
    QObject::connect(bioboard, SIGNAL(changeTabParent(int)), this, SLOT(changeTabChild(int)));
}

void Biotab::changeTabChild(int direction) {
    emit changeTabParent(direction);
}

void Biotab::paintEvent(QPaintEvent *) {
    ...
    // ooops
    painter.drawText(120, 200, QString::fromUtf8("Ooops!"));
